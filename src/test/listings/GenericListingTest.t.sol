...
import {Proposal} from "../../address-book/IPGovernance.sol";
import {IPGovernance, IPEthereum} from "../../address-book/IPAddressBook.sol";

contract GenericListingTest is Test {
    address public constant IPT_WHALE = 0x3Df70ccb5B5AA9c300100D98258fE7F39f5F9908;
...
contract Propose is GenericListingTest {
    ...
    function test_createsProposal() public {
        ...
        vm.startPrank(0x3Df70ccb5B5AA9c300100D98258fE7F39f5F9908);
        console.log(msg.sender);
        console.log(IPT_WHALE);
        GenericListing.propose(
            GenericListing.ProposalData({
                targets: targets,
                values: values,
        ...
        // Post-proposal assertions
        assertEq(IPGovernance.GOV.proposalCount(), 20);
        Proposal memory proposal = IPGovernance.GOV.proposals(20);
        assertFalse(proposal.emergency);
        assertEq(proposal.id, 20);
    }
}
